...
obtained here: https://github.com/trishprabhu/policyorpartisanship. The original 
data/code is cited in the "References" section below.] the authors' main results 
...
more nuanced picture (e.g. a better understanding of uncertainty, etc.) of the 
results.
...
area of investigation), have a binary outcome/dependent variable (partyswitcher 
...
The results above suggest that in the case of Table 1, the authors didn't 
necessarily have much to gain substantively in utilizing logistic regression 
instead of linear regression; neither the significance nor the direction of any 
of the coefficient estimates change. With that said, we do get a clearer, more 
nuanced picture of the results with an odds ratio interpretation; specifically,
it more concretely allows us to compare the relative magnitudes of the various 
coefficients. The same is true in the case of the probability values: while, 
across both linear and logistic, the significance of the probability values stay 
the same, we do see that the logistic probability estimates vary a bit more
in magnitude, suggesting additional nuance. Indeed, both the logistic odds
ratios and probability estimates have larger standard errors; this implies
that the logistic regression ultimately captures and reflects more uncertainty, 
a key area of "value-add" in interpreting the results.

^*Change this to include additional discussion of the larger SEs (numerical,
as seen in the table) associated with the logistic regression!
...
at the 0.05 level across both models). This is likely due to the fact that (as 
is visually apparent) for both the odds ratios and probabilities, once again, 
the logistic regression values have larger standard errors; the model clearly 
captures and reflects additional uncertainty. *Add onto this!

Ultimately, then, like Table 1,
the "value-add" of the logistic regression is in the additional nuance around/
...
* While logistic regression didn't necessarily produce a change in the 
substantive results of the paper, it did add additional nuance/understanding to
interpretation of the results; in particular, the odds ratios/probability values
associated with the logistic regression model captured/reflected additional
uncertainty via larger standard errors, a key area of "value-add."
