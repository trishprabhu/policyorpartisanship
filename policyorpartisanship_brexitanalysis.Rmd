...
# NOTE FOR TRISHA: You need to change the names of the variables in the table
# to match the paper!

table1 <- stargazer(table1reg, intreg1, intreg1a, 
          title = "Euroskepticism and Defection from the Conservatives", 
          no.space = TRUE,star.cutoffs = c(0.05, 0.01, 0.001)) 
table1

# How do I get rid of the weird author caption being produced with the table?
...
table2reg <- lm(merge2$switchtocons ~ merge2$EUIntegrationSelf8)
summary(table2reg)
...
# From the paper: "the interaction is negative and significant: less Euroskeptic 
# Conservatives who perceive that the party has become increasingly Euroskeptic 
# are especially likely to reject it.

# And now, let's throw everything in again:

intreg2a <- lm(merge2$switchtocons ~ merge2$EUIntegrationSelf8 * merge2$Conchange 
              + merge2$age.x + merge2$gender.x + merge2$white + 
                as.factor(merge2$country.x))
summary(intreg2a)
